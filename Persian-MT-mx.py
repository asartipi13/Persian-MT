...
def predict(df_test, model, src, tar):
    ...
    number_of_batches_in_test_df = [i for i in range(number_of_batches_in_test_df)]
    # for i in range(number_of_batches_in_test_df):
    for i in tqdm(number_of_batches_in_test_df):
...
def train_process(src, tar, model_dir, logger):
    ...
    if need_train:
        n_epoch = [i for i in range(n_epochs)]

        try:
            for epoch_idx in tqdm(n_epoch):
                train_losses = []
                dev_losses = []
